...
function Resist() {
  ...
  return (
    <div className={styles.regist}>
      ...
      <span>
				<h2 style={{margin: '0'}}>프로젝트 이름 : <input type="text" value={name} onChange={e=>setName(e.target.value)}></input></h2>
			<div>
      ...
      <span>
        ...
        <p>기술 스택 : <input type="text" value={eachSkill} onChange={e=>setEachSkill(e.target.value)}></input>
				<button onClick={skillbutton}>클릭</button></p>
        ...
        <li>기술스택1 : {skill[0]}</li>
				...
				<h3>프로젝트 개요 : <input type="text" value={comment} onChange={e=>setComment(e.target.value)}></input></h3>
			
      <div className={styles.btn}>
				
        <span>
        <button onClick={createDatabase}>등록하기</button></span>
        <span style={{paddingLeft: '10px'}}>
        <button onClick={() => history.push('/')}>취소</button></span>
			</div>
			<div>
				<img src={getImage}/>
			</div>
