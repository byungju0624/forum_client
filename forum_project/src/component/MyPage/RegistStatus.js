...
//-------------------------------------------------------------------------------------------- 데이터 불러와 서 프론트 팀이 원하는 형식으로 가공
	...
	let getApprovedSubmmit = async (projectName, user) => {
		console.log("버튼을 눌렀을 때 들어오는 유저 아이디 값은 다음과 같다 : "+user)
		console.log("버튼을 눌렀을 때 들어오는 프로젝트 이름 값은 다음과 같다 : "+projectName)
	...
	let ProjectDataUpdate = (projectName, user) => {
		firestore.collection("project").doc(projectName).update({
	...
	let applyUserDataApprovedUpdate = (projectName, user) => {
		firestore.collection("users").doc(user).update({
			...
			message : firebase.firestore.FieldValue.arrayUnion(`프로젝트 ${projectName}에 참가가 승인 되었습니다`)
		})
	}
	...
	let deleteSubmmitApprovedData = (host, user, projectName) => {
		...
		firestore.collection("users").doc(host).update({
			submittedProject : firebase.firestore.FieldValue.arrayRemove(objectDelete)
		})
	}
	...
	let rejectUserDataApprovedUpdate = (projectName, user) => {
		firestore.collection("users").doc(user).update({
			appliedProject : firebase.firestore.FieldValue.arrayRemove(projectName),
			message : firebase.firestore.FieldValue.arrayUnion(`프로젝트 ${projectName}에 참가가 거절 되었습니다`)
		})
	}
  ...
  return (
    <div className={styles.header}>
      {submittedProjectData.map((el) => {
				console.log("상태체크를 해보고 싶어요 : "+JSON.stringify(submittedProjectData))
                ...
                <div className={styles.btn}>
                  {el[projectName].map((data) => {
                    return (
                      <frameElement>
                        <div>
                          <button style={{ fontWeight: "bold" }} onClick={()=>{getApprovedSubmmit(projectName,data)}}>승인</button>
													<button style={{ marginLeft: "5px", fontWeight: "bold" }} onClick={()=>{getRejectSubmmit(projectName,data)}}>거절</button>
                        </div>
                      </frameElement>
                    );
                  })}
