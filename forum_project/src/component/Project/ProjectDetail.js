...
function ProjectDetail() {
  ...
  let party = dataFire[0].party;
	...
	let readProjectData = (account) => {
		firestore.collection("users").doc(account).get().then(async function(doc){
			if(doc.exists){
	...
	let applyProject = async() => {
		...
		if(account === undefined){
			...
			await firestore.collection("users").doc(host).update({
    ...
    <>
      <div className={styles.projectDetail}>
        ...
        <div className={styles.btn}>
          ...
          <span>
            <button style={{ marginLeft: "20px" }} onClick={applyProject}>지원하기</button>
          </span>
          <span>
            <button
